...
- 支持文本, 图片, 文件, 语音四种消息;
- 开启peer_mode 配置后, 将加入一个虚拟的Relay Network, 用户可以发现和在不同peer间漫游;
...
- 匿名性
  ...
  + 服务器不保存任何与用户身份有关的数据;
- 安全性
  + 群组及私聊的文本, 图片, 语音消息均采用流行且高效的XChacha20-poly1305算法加密, 该算法是开源的,采用的库来自[RustCrypto](https://github.com/RustCrypto)
  + 每条消息采用随机的nonce;
  ...
  + 客户端采用的关键安全操作,封装在WebAssembly中(同样开源), 以增强安全性和性能;
...
- 如何拉黑/屏蔽某个用户?
  ...
  + 如果多个人block了同一用户, 累计超过一定次数(可配置), 给用户将被banned, 需等待指定的时间后, 才能连接节点;
